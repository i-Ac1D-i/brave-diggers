...
chat_network.__index = chat_network

function chat_network.New()
    return setmetatable({}, chat_network)
end

function chat_network:Init()
    ...
    if not self.schedule_id then
        self:RegisterProto()
        self.schedule_id = cc.Director:getInstance():getScheduler():scheduleScriptFunc(function(elapsed_time)
            self:Update(elapsed_time)
        end, 0, false)
    end
end

function chat_network:RegisterProto()
    local buff = aandm.getDataFromFile("proto/msg.pb")
    protobuf.register(buff)
end

function chat_network:SetTime(time)
    self.cur_time = time
    self.next_heart_beat_time = self.cur_time + HEART_BEAT_DELAY
end

function chat_network:Clear()
    self:Disconnect()
    self.last_content = ""
    self.socket = nil
...
function chat_network:Connect(ip, port)
...
function chat_network:IsConnected()
    return self.status == NET_STATUS["connected"]
end

function chat_network:Disconnect(net_status)
    if self.socket then
        self.socket:close()
    end
...
function chat_network:Update(elapsed_time)
    ...
    if self.status == NET_STATUS["connected"] then
        ...
        if result then
            ...
            local msg = protobuf.decode2("CHAT2CILENT", result)
            local new_session
...
function chat_network:Send(msg, ignore_session)
    ...
    local t = protobuf.encode("CLIENT2CHAT", msg)
    local size = #t
...
function chat_network:HeartBeat()
    if not self:IsConnected() then
        return
    end
    ...
    if self.waiting_heart_beat_response then
        ...
        local t = protobuf.encode("CLIENT2CHAT", { heart_beat = {}} )
        local size = #t
...
function chat_network:RegisterEvent(msg_name, handler)
    self.event_name_list[msg_name] = handler
    self.event_listener:Register(msg_name, handler)
end

function chat_network:HasLostConnection()
