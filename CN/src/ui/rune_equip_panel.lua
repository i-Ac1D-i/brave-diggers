...
function rune_equip_panel:Init(root_node)
    ...
    if not feature_config:IsFeatureOpen("rune_exchange") and platform_manager:GetChannelInfo().hide_rune_exchange_btn then
...
function rune_equip_panel:RegisterWidgetEvent()
    ...
    self.draw_btn:addTouchEventListener(function(widget, event_type)
        if event_type == ccui.TouchEventType.ended then
            audio_manager:PlayEffect("click")
            if feature_config:IsFeatureOpen("rune_exchange") then
                graphic:DispatchEvent("show_world_sub_scene", "rune_exchange_sub_scene", constants["SCENE_TRANSITION_TYPE"]["none"], true)
            else
