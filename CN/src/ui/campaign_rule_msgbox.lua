...
function campaign_rule_msgbox:Init()
    ...
    --是否需要重新排版
    if channel_info.rule_panel_rest_view then
        self:Composing() 
    else
        self.rank_num_texts = {}
...
function campaign_rule_msgbox:ShowExt1()
    ...
    for k,v in pairs(campaign_logic.top_score_list) do
        local rank_title_text = self.rank_title_texts[k]

        if rank_title_text then
            if v.min == v.max then
                rank_title_text:setString(string.format(lang_constants:Get("campaign_daily_reward1"), v.min))
            else
                rank_title_text:setString(string.format(lang_constants:Get("campaign_daily_reward2"), v.min, v.max))
            end

            self.rank_num_texts[k]:setString(" x"..v.value)
        end
    end
    ...
    for k, v in pairs(campaign_logic.special_cond_list) do
        for i, vv in ipairs(v.and_list) do
            self.mercenary_desc:setString(lang_constants:GetCampaignCond(vv.type,vv.value))
            break
        end
    end
...
function campaign_rule_msgbox:ShowExt2()
    self.root_node:setVisible(true)
