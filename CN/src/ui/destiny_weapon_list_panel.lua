...
function weapon_sub_panel:Show(id, is_actived, is_equiped)
    self.weapon_icon_img:loadTexture(destiny_skill_config[id]["icon"], PLIST_TYPE)
    self.name_text:setString(destiny_skill_config[id]["name"])
    self.equiped_icon_img:setVisible(false)

    local skill_id = destiny_logic:GetCurWeaponSkillId(id)
...
function destiny_weapon_list_panel:Init()
    ...
    if feature_config:IsFeatureOpen("expedition_and_destiny_weapon") then
        ...
        self.weapon_sub_panels[id] = weapon_sub_panel.New()
        self.weapon_sub_panels[id]:Init(template:clone())
        self.list_view:addChild(self.weapon_sub_panels[id].root_node)
    end
...
function destiny_weapon_list_panel:Show(formation_id)
    ...
    for id,sub_panel in ipairs(self.weapon_sub_panels) do
        local is_actived = destiny_logic:IsWeaponActived(id)
        sub_panel:Show(id, is_actived, self.cur_equiped_weapon_id == id)
    end
end
...
function destiny_weapon_list_panel:RegisterWidgetEvent()
   ...
   local view_weapon_info = function (widget, event_type)
    ...
    for i,sub_panel in ipairs(self.weapon_sub_panels) do
        local sub_panel = sub_panel.root_node
