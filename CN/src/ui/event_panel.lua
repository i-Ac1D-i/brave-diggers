...
function dialog_sub_panel:Init(root_node)
    ...
    self.paragraph_text:getVirtualRenderer():setMaxLineWidth(TEXT_MAX_LENGTH)
...
function dialog_sub_panel:Show(str, index)
    ...
    local max_text_length
    local char_width
    local channel_info = platform_manager:GetChannelInfo()
    if channel_info.meta_channel == "txwy_dny" then
        char_width = channel_info.chars_width[platform_manager:GetLocale()]
        max_text_length = math.ceil(TEXT_MAX_LENGTH/char_width)
    ...
    elseif platform_manager:GetLocale() == "jp" then
        ...
        max_text_length = math.floor(TEXT_MAX_LENGTH/char_width)  --FYD  注意：由于日文字体并不是每个字符都一样大的，所以这个每行容纳的字符个数就不能是固定的
    ...
    if len >= max_text_length then
        ...
        self.real_height = 107 + (line_num - 1) * 30

        self.bg_img:setContentSize({ width = 446, height = self.real_height })
        self.paragraph_text:setContentSize({width = TEXT_MAX_LENGTH, height = self.real_height })
        ...
        self.paragraph_text:setContentSize({width = math.min(len*char_width, TEXT_MAX_LENGTH), height = 128})
    ...
    self.formation_btn:addTouchEventListener(function(widget, event_type)
        if event_type == ccui.TouchEventType.ended then
            ...
            if self.cave_event_flag then 
                ...
                graphic:DispatchEvent("show_world_sub_scene", "formation_sub_scene", trans_type, mode, back_panel, nil)
            end
        end
    end)
