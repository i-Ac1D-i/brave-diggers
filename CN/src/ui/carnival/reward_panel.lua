...
function reward_panel:Init(root_node)
    ...
    if platform_manager:GetChannelInfo().reward_pannel_item_text_width_append then
        local origin_size = self.desc_text:getContentSize()
        origin_size.width = origin_size.width + 100 
        self.desc_text:setContentSize(origin_size)
    end
...
function reward_panel:Show(config)
    ...
    elseif cur_reward_type == CARNIVAL_REWARD_TYPE["single"] then
        ...
        self.desc_text:setString(carnival_logic:GetLocaleInfoString(conf, "name") .. " x " .. reward_info.param2)
    ...
    elseif cur_reward_type == CARNIVAL_REWARD_TYPE["multi"] then
        ...
        self.icon_panel:ShowTextBg(false)
        self.desc_text:setString(carnival_logic:GetLocaleInfoString(self.config, "mult_str2", 1))
    ...
    elseif cur_reward_type == CARNIVAL_REWARD_TYPE["token"] then
        ...
        self.desc_text:setString(string.format(lang_constants:Get("carnival_item"), carnival_logic:GetLocaleInfoString(conf, "name")))
    ...
    elseif cur_reward_type == CARNIVAL_REWARD_TYPE["vote"] then
        ...
        self.desc_text:setString(carnival_logic:GetLocaleInfoString(conf, "name") .. "x" .. cur_value)
        self.vote_desc_text:setVisible(true)

        local info = carnival_logic:GetCarnivalInfo(self.config.key)
        self.vote_desc_text:setString(string.format(lang_constants:Get("carnival_vote_num_desc"), carnival_logic:GetLocaleInfoString(info, "cur_value_multi", 2), carnival_logic:GetLocaleInfoString(conf, "name")))
