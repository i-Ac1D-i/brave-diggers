...
{
    [1] = "carnival_payment_first_gift",
    [2] = "take_vip_reward",
    [3] = "ladder_already_get_reward",
}
...
function carnival_limit_panel:Show(config)
    ...
    elseif self.config.carnival_type == constants.CARNIVAL_TYPE["first_payment"] then
        ...
        self.first_payment_text:setString(lang_constants:Get(FIRST_PAYEMNT_PROMPT[status]))
    end

    self.title_text:setString(carnival_logic:GetLocaleInfoString(self.config, "name"))
    self.name_text:setString(carnival_logic:GetLocaleInfoString(self.config, "mult_str1", self.cur_index))
    self.desc_text:setString(carnival_logic:GetLocaleInfoString(self.config, "mult_str2", self.cur_index))
