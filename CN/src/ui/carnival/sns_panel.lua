...
function invitation_sub_panel:Load()
    ...
    self.desc_text:setString(string.format(carnival_logic:GetLocaleInfoString(self.config, "mult_str1", str_index), self.config.mult_num2[self.step_index] , self.config.mult_num1[self.step_index]))
end
...
function txwy_invite_panel:Init(root_node)
    ...
    self.rule_icon = self.copy_btn:getChildByName("rule_icon") 
    local copy_str = lang_constants:GetFormattedStr("sns_copy_str")
    if copy_str then
       self.copy_btn:setTitleText(copy_str) 
    end
    ...
    bg:setVisible(false)
    ----------UI初始化完毕-------
    --如果是txwy，那么在初始化ui之后对ui以及逻辑进行处理
    local info =  platform_manager:GetChannelInfo()
    if info.meta_channel == "txwy" or info.meta_channel == "txwy_dny" then
            ...
            self:GeneralInviteLinkURL(function(url)    
            self.invite_link_text:setString(url)   
        end)
        if self.rule_icon then
            self.rule_icon:addTouchEventListener(function(widget, event_type)
                if event_type == ccui.TouchEventType.ended then
                    audio_manager:PlayEffect("click")
                    graphic:DispatchEvent("show_world_sub_panel", "sns_rule_panel") 
                end
            end)
        end   
...
function txwy_invite_panel:GeneralInviteLinkURL(callback)
    local info = platform_manager:GetChannelInfo()
    if info.fb_link_url then
        txwy_invite_panel.url = info.fb_link_url
    end
    ...
    local url = info.app_pre_link_url .. info.app_link_url
    if info.fb_channel then
        url = url .. info.fb_channel
    end
...
function sns_panel:Show()
    if self.channel.meta_channel == "txwy" then
        self:LoadTXWY()
    elseif self.channel.meta_channel == "txwy_dny" then
        self:LoadTXWYDNY()
    ...
    self.duration_text:setString(carnival_logic:GetLocaleInfoString(self.config, "duration"))
    self.title_text:setString(carnival_logic:GetLocaleInfoString(self.config, "name"))
    self.desc_text:setString(carnival_logic:GetLocaleInfoString(self.config, "desc"))
...
function sns_panel:LoadTXWYDNY()
...
function sns_panel:LoadR2GAMES()
    ...
    if not sns_logic:ShowFBLikeBtn(params) then   
...
function sns_panel:RegisterWidgetEvent()
    ...
    self.fb_share_btn:addTouchEventListener(function(widget, event_type)
        if event_type == ccui.TouchEventType.ended then
            audio_manager:PlayEffect("click")
            local channel_info = platform_manager:GetChannelInfo()
            if channel_info.meta_channel == "txwy" or channel_info.meta_channel == "txwy_dny" then
                local message = channel_info.share_message
                txwy_invite_panel:GeneralInviteLinkURL(function(url)
