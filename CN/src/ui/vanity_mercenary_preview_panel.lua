...
function mercenary_preview_panel:Init(mode, parent_node)
    ...
    self.skill_desc_text = skill_desc_bg_node:getChildByName("desc")
     ...
     --FYD  将技能描述换成可移动的  
    local txt_height = platform_manager:GetChannelInfo().mercenary_preview_panel_desc_txt_height
    local font_height = platform_manager:GetChannelInfo().mercenary_preview_panel_desc_txt_font_height
    if txt_height and font_height then
        
        local origin_size = self.skill_desc_text:getContentSize() 
        self.skill_desc_text:setContentSize(cc.size(origin_size.width,txt_height)) 
        self.scroll_view = ccui.ScrollView:create()
        self.scroll_view:setDirection(1) 
        local innerContainer = self.scroll_view:getInnerContainer()
        innerContainer:ignoreAnchorPointForPosition(true) 
        innerContainer:setAnchorPoint(cc.p(0,1)) 
        self.scroll_view:setContentSize(origin_size)   
        self.scroll_view:setInnerContainerSize(cc.size(origin_size.width,80))     
        self.scroll_view:setPosition(cc.p(self.skill_desc_text:getPosition())) 
        self.scroll_view:setAnchorPoint(cc.p(0,1))
        local parent = self.skill_desc_text:getParent()
        parent:removeChild(self.skill_desc_text)
        self.skill_desc_text:setPosition(cc.p(0,0)) 
        self.scroll_view:addChild(self.skill_desc_text) 
        parent:addChild(self.scroll_view)  
        self.scroll_view:scrollToTop(0,false) 

        local origin_func =  self.skill_desc_text.setString 
        self.skill_desc_text.setString = function(obj,msg) 
            print('FYD MSG = '..msg) 
            origin_func(self.skill_desc_text,msg)

            local nums = self.skill_desc_text:getVirtualRenderer():getStringNumLines()
            if platform_manager:GetChannelInfo().is_open_system then
                local size = self.skill_desc_text:getAutoRenderSize()
                local content_size = self.skill_desc_text:getVirtualRenderer():getContentSize()
                nums = math.ceil(size.width / content_size.width) + 1
            end 
            local height = nums * font_height
            if height > 80 then
...
function mercenary_preview_panel:Show(mercenary_id, formation_id)
    local mercenary = troop_logic:GetVanityMercenarys(mercenary_id)
    ...
    if mercenary.contract_lv and mercenary.contract_lv ~= 0 then
...
function mercenary_preview_panel:ShowMercenarySkillDesc(skill_type, pos)
    ...
    if self.is_leader then
        ...
        elseif skill_type == SKILL_TYPE["contract_skill"] then
            self.coop_and_artifact_text:setVisible(false)
            local mercenary = troop_logic:GetVanityMercenarys(self.mercenary_id)
