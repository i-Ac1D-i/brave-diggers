...
function escort_tramcar_panel:Init(root_node)
    ...
    self.buy_escort_icon = self.root_node:getChildByName("add_time")
    ...
    self.refresh_random_tramcar_cost:setString(constants["ESCORT_REFRESH_TRAMCAR_RANDOM_COST"])
end
...
function escort_tramcar_panel:RefreshRemainFreeTimes()
    if constants["FREE_REFRESH_TRAMCAR_TIMES"] > escort_logic:GetRefreshTramcarTimes() then
        ...
        if feature_config:IsFeatureOpen("review") then
            ...
            -- 是否需要显示 提示
            local is_need_show = constants["FREE_REFRESH_TRAMCAR_TIMES"] <= escort_logic:GetRefreshTramcarTimes()

            if not resource_logic:CheckResourceNum(constants["RESOURCE_TYPE"]["blood_diamond"], constants["ESCORT_REFRESH_TRAMCAR_RANDOM_COST"], false,is_need_show) then 
                self.refresh_random_tramcar_btn:setColor(panel_util:GetColor4B(0x7f7f7f))
                self.refresh_random_tramcar_icon:setColor(panel_util:GetColor4B(0x7f7f7f))
            else
                self.refresh_random_tramcar_btn:setColor(panel_util:GetColor4B(0xffffff))
...
--刷新购买运送次数按钮
function escort_tramcar_panel:RefreshBuyEscortBtn()
    if feature_config:IsFeatureOpen("review") then
        self.buy_escort_btn:setVisible(false)
        self.buy_escort_icon:setVisible(false)
    else
        self.buy_escort_btn:setVisible(true)
        self.buy_escort_icon:setVisible(true)
    end
end
        ...
        if event_type == ccui.TouchEventType.ended then
            ...
            if not self.show_selected_animation then
                ...
                if escort_info.tramcar_id ~= constants["SPECITY_TRAMCAR_ID"] then
                    ...
                    elseif feature_config:IsFeatureOpen("review") then
