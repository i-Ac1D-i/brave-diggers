...
local event_tooltip_panel = panel_prototype.New()
function event_tooltip_panel:Init(root_node)
    self.root_node = root_node
...
function event_tooltip_panel:Show()

    if user_logic:IsFeatureUnlock(client_constants.FEATURE_TYPE["mining_random_event"], false) then
...
function event_tooltip_panel:RegisterWidgetEvent()
...
function golem_tooltip_panel:Load()
    ...
    local index = 2
    if has_nightmare then
        self.block_info_sub_panels[2]:Load(CONST_BLOCK_TYPE["golem_nightmare"], 180)
        index = 3
    end

    for i = 3, valid_sub_panel_num do
    ...
    self.root_node:addTouchEventListener(function(widget, event_type)
        if event_type == ccui.TouchEventType.began then
            ...
            if feature_config:IsFeatureOpen("mining_random_event") then
                self.root_node:getParent():getChildByName("golem_info2"):setOpacity(50)
            end
        elseif event_type == ccui.TouchEventType.ended or event_type == ccui.TouchEventType.canceled then
            self.tooltip_img:setVisible(false)
            self.arrow_img:setRotation(90)
            ...
            if feature_config:IsFeatureOpen("mining_random_event") then
                self.root_node:getParent():getChildByName("golem_info2"):setOpacity(255)
            end
        end
    end)
end
...
function mining_district_panel:Init(fix_block_sprite)
    FIX_BLOCK_SPRITE = fix_block_sprite
    ...
    if feature_config:IsFeatureOpen("mining_random_event") then
        event_tooltip_panel:Init(self.root_node:getChildByName("golem_info2"))
        self.root_node:getChildByName("golem_info2"):getChildByName("tooltip"):setVisible(false)
        
        self.reward_bg = self.root_node:getChildByName("reward_bg")
...
function mining_district_panel:RefreshDigCount()
...
function mining_district_panel:RegisterEvent()
    ...
    graphic:RegisterEvent("update_dig_count", function()
        if not self.root_node:isVisible() then
            return
        end
