...
local WORLD_PANEL_SHOW_MODE_LIST =
{
    ...
    ["mercenary_vanity_choose_sub_scene"] =  WORLD_PANEL_SHOW_MODE["show_both"],
    ...
    ["mine_sub_scene"] =  WORLD_PANEL_SHOW_MODE["show_both"],
    ["mine_report_sub_scene"] = WORLD_PANEL_SHOW_MODE["show_both"],
    ["carnival_exchange_sub_scene"] =  WORLD_PANEL_SHOW_MODE["show_both"],
    ["server_pvp_sub_scene"] = WORLD_PANEL_SHOW_MODE["show_both"],
    ["guild_boss_sub_scene"] =  WORLD_PANEL_SHOW_MODE["show_both"],
    ["guild_member_sub_scene"] =  WORLD_PANEL_SHOW_MODE["show_both"],
    ["share_sub_scene"] =  WORLD_PANEL_SHOW_MODE["show_both"],
    ["ladder_tournament_sub_scene"] = WORLD_PANEL_SHOW_MODE["show_both"],
    ["destiny_weapon_star_sub_scene"] =  WORLD_PANEL_SHOW_MODE["show_both"],
    ["evolution_sub_scene"] =  WORLD_PANEL_SHOW_MODE["show_both"],
    ["evolution_final_sub_scene"] =  WORLD_PANEL_SHOW_MODE["hide_both"],
    ["resource_recycle_sub_scene"] = WORLD_PANEL_SHOW_MODE["show_both"],
    ["rune_exchange_sub_scene"] = WORLD_PANEL_SHOW_MODE["show_both"],
    ["vanity_adventure_sub_scene"] = WORLD_PANEL_SHOW_MODE["show_both"],
    ["vanity_main_sub_scene"] = WORLD_PANEL_SHOW_MODE["show_both"],
}
...
local BACK_TO_CHOOSE_UI_BTN = 
{
    ...
    ["destiny_weapon_star_sub_scene"] =  TAB_TYPE["mercenary"],
    ...
    ["mercenary_cultivation_sub_scene"] = {},
    ...
    ["payment_sub_scene"] = {},
    ["campaign_sub_scene"] = {},
    ["ladder_tournament_sub_scene"] = {},
    ["evolution_sub_scene"] = {},
    ["resource_recycle_sub_scene"] = {},
    ["rune_exchange_sub_scene"] = {},
    ["vanity_adventure_sub_scene"] = {},
    ["mercenary_vanity_choose_sub_scene"] = {},
    ["vanity_main_sub_scene"] = {},
}
...
function world_panel:Show(cur_sub_scene_name)
    ...
    panel_util:ConvertUnit(troop_logic:GetTroopBP(), self.bp_text, self.bp_unit_text)
...
function world_panel:Update(elapsed_time)
    ...
    if locale == "zh-CN" or locale == "zh-TW" or locale == "th" or locale == "vi" then 
...
function world_panel:RegisterWidgetEvent()
    local change_sub_scene = function(touch, event,new_tab_index) 
        local tab_index
        if not new_tab_index then  -- 资源跳转
        ...
        elseif tab_index == TAB_TYPE["recruit"] then
            if user_logic:IsFeatureUnlock(FEATURE_TYPE["recruit"]) then
                graphic:DispatchEvent("hide_magic_shop_pannel")
    ...
    -- 资源跳转
    graphic:RegisterEvent("update_world_tab", function(index) 
        change_sub_scene(nil,nil,index) 
    end)
...
function world_panel:RegisterEvent()
    graphic:RegisterEvent("update_resource_list", function()
        ...
        if resource_logic:IsResourceUpdated(RESOURCE_TYPE["blood_diamond"]) then
            if not rune_logic:GetDrawState() then
                self.blood_diamond_text:setString(resource_list.blood_diamond)
            end
        end
    ...
    --更新血钻 增加量
    graphic:RegisterEvent("update_blood_diamond", function(add_blood_diamond)
        if not self.root_node:isVisible() then
            return
        end
        local blood_diamond = tonumber(self.blood_diamond_text:getString())
        if add_blood_diamond == nil then
            local resource_list = resource_logic:GetResourceList()
            blood_diamond = resource_list.blood_diamond
        else
            blood_diamond = blood_diamond + add_blood_diamond
        end
        self.blood_diamond_text:setString(blood_diamond)
    end)
end
