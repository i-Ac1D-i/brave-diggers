...
function msgbox:Init(root_node)
    ...
    self.mask_node:setContentSize(1280, 2272)
    self.mask_node:setBackGroundColorOpacity(0)
    self.mask_node:setTouchEnabled(true)
    self.mask_node:setPosition(-self.mask_node:getContentSize().width/2,-self.mask_node:getContentSize().height/2)
    self.mask_node:setVisible(true)
    self.bg:addChild(self.mask_node,1)
...
function msgbox:Show(title, desc, confirm_txt, close_txt, callback, close_callback, close_callback2)
    ...
    self.close_callback2 = close_callback2
