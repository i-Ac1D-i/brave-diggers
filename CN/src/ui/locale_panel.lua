...
function locale_panel:RegisterWidgetEvent()
    ...
    self.confirm_btn:addTouchEventListener(function(widget, event_type)
        if event_type == ccui.TouchEventType.ended then
            ...
            lang_constants:Init(cur_locale,platform_manager:GetChannelInfo()) 
            ...
            self:Hide()
            if platform_manager:GetChannelInfo().meta_channel == "txwy_dny" and PlatformSDK.setLanguage then
                local locale = lang_constants["CONVERT_LANG"][cur_locale] or cur_locale  --FYD
                PlatformSDK.setLanguage(locale) 
            end
