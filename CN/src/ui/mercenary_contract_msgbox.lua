...
function cost_sub_panel:Init(root_node, tag)
    ...
    self.desc_text:getChildByName("shadow"):setVisible(feature_config:IsFeatureOpen("craft_soul_stone"))
    self.desc_text:getChildByName("arrow"):setVisible(feature_config:IsFeatureOpen("craft_soul_stone"))
...
function cost_sub_panel:Load(conf, tag)
    local use_soul_bone = platform_manager:GetChannelInfo().mercenary_contract_use_soul_bone
    ...
    if template_id ~= 0 then
        ...
        return is_enough
    elseif use_soul_bone and mercenary_contract_msgbox.contract_lv == 1  and tag == 2 then
    ...
    elseif use_soul_bone and mercenary_contract_msgbox.contract_lv == 2  and tag ~= 3 then
        local confs 
        if tag == 1 then 
...
function cost_sub_panel:CheckSoulStone(template_id, check_num)
    ...
    for i = 1, 7 do
        ...
        if not resource_logic:CheckResourceNum(constants.RESOURCE_TYPE[resource_name], cost_resource * check_num, false,true) then -- 资源跳转
...
function mercenary_contract_msgbox:Load(conf)
    ...
    local _, resource_is_enough = panel_util:LoadCostResourceInfo(conf, self.resource_sub_panels, 50, MAX_SUB_PANEL_NUM, center_x,true)  -- 资源跳转
...
function mercenary_contract_msgbox:RegisterWidgetEvent()
    ...
    if feature_config:IsFeatureOpen("craft_soul_stone") then
        for i = 1, constants["MAX_CONTRACT_SOUL_TYPE"] do
