...
local MAIL_PANEL_TYPE = client_constants.MAIL_PANEL_TYPE
...
function mail_sub_panel:Show(mail)
    ...
    elseif mail.mail_type == MAIL_TYPE["mercenary"] then
        name = mercenary_config[template_id]["name"]
        source_type = constants.REWARD_TYPE["mercenary"]
        self.desc_text:setString(name .. "x" .. mail.extra_num)
    ...
    elseif mail.mail_type == MAIL_TYPE["rune"] then
        ...
        self.desc_text:setString(name)
    elseif mail.mail_type == MAIL_TYPE["title"] then
        local name = config_manager.title_config[template_id].title_name
        titile_name = string.format(lang_constants:Get("title_mail_name"),name) 
        self.desc_text:setString("")
        take_text = lang_constants:Get("title_commit")
    end
    ...
    if mail.mail_type == MAIL_TYPE["title"] then
        self.icon_template.root_node:setVisible(false)
        self.count_down_text:setVisible(false)
        self.desc_text:setVisible(false)
        self.name_text:setString(titile_name)
        self.name_text:setPosition(cc.p(30,60))
        self.send_text:setVisible(false)
    else
        self.icon_template.root_node:setVisible(true)
        self.count_down_text:setVisible(true)
        self.desc_text:setVisible(true)
        self.name_text:setPosition(self.name_text_origin)
    end
    ...
    if mail.mark_read then
        ...
        if take_text then
            self.take_btn:setTitleText(take_text)
        else
...
function mail_panel:Init()
    ...
    self.system_num_text = self.system_mail_btn:getChildByName("txt")
    self.friend_num_text = self.friend_mail_btn:getChildByName("txt")
