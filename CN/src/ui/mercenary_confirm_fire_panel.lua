...
function mercenary_confirm_fire_panel:Init()
   ...
   self.tobe_fired_num_text = temp_info_node:getChildByName("choose_num")
    ...
    if feature_config:IsFeatureOpen("contract_soul_bone") then
        for i = 1 , 6 do
            self["soul_chip_" .. i-1] = temp_info_node:getChildByName("soul_chip_" .. i-1)
        end
    else
        self.contract_exp_text = temp_info_node:getChildByName("contract_exp")
    end
    ...
    if not channel.is_close_contract_2 then
        local titleInof=temp_info_node:getChildByName("soul_chip_title_0")
...
function mercenary_confirm_fire_panel:Show(fire_num, soul_chip, exp, tobe_fired_id_list, contract_exp, soul_bone, quality)
    ...
    if platform_manager:GetChannelInfo().mercenary_contract_use_soul_bone then
        ...
        self.contract_exp_text:setString(contract_exp)
    end
