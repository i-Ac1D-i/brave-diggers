...
function goods_info_sub_panel:Init(root_node, product, index)
    ...
    self.doubleText = root_node:getChildByName("Image_27")
    if self.doubleText then
        self.doubleText:setVisible(false)
        self.doubleText:setAnchorPoint(cc.p(0,0.5))
    end
    ...
    elseif platform_manager:GetChannelInfo().meta_channel == "txwy_dny" then
        --东南亚颜色值修改
        self.desc_text:setColor(panel_util:GetColor4B(client_constants.TEXT_QUALITY_COLOR[7]))
    else
...
function goods_info_sub_panel:Show(product)
    ...
    if product.type == constants.PAYMENT_PRODUCT_TYPE["adventure_vip"] then
        ...
        if feature_config:IsFeatureOpen("store_double_reward") then
            desc = lang_constants:Get("payment_double_card")
            self.desc_text:setVisible(true)
        end
        ...
        if payment_logic:GetPurchasedMark(product.product_id) == 0 and product.gift > 0 then
            ...
            if product.gift2 > 0 then
                ...
                if feature_config:IsFeatureOpen("store_double_reward") then
                    desc = lang_constants:Get("payment_double_blood")
                end
            else
                desc = product.desc
            end
    ...
    if self.doubleText then
        if feature_config:IsFeatureOpen("store_double_reward") then
            self.limit_img:setVisible(true)
            self.limit_text:setString("on sale 50%")
            self.limit_text:setVisible(true)
            self.doubleText:setVisible(true)
            self.doubleText:setPosition(cc.p(self.name_text:getPositionX() + self.name_text:getContentSize().width, self.name_text:getPositionY()))
        end
    end
...
function payment_panel:ShowMoreBtn()
    local is_show = true
    if self.recharge_btn then
        local is_unlock = adventure_logic:IsMazeClear(open_id)
        if platform_manager:GetChannelInfo().meta_channel == "txwy_dny"  then 
            --东南亚要关卡开启条件
            is_show = is_unlock
            if not feature_config:IsFeatureOpen("third_party_recharge") then
                is_show = false
            end
            --语言开关
            local locale = platform_manager:GetLocale()
            if locale == "en-US" or locale == "zh-CN" or locale == "zh-TW" then
                --不要显示的语言
                is_show = false
            end
        end

        if platform_manager:GetChannelInfo().has_recharge_btn and is_show then
            
            if platform_manager:GetChannelInfo().meta_channel == "txwy_dny" and self.recharge_btn:getChildByName("Text_30") then 
                self.recharge_btn:setTitleText("")
                self.recharge_btn:getChildByName("Text_30"):setString(lang_constants:Get("more_pey_btn_name"))
                self.recharge_btn:setContentSize(cc.size(self.recharge_btn:getChildByName("Text_30"):getContentSize().width + 30, self.recharge_btn:getContentSize().height))
            else   
