...
function info_sub_panel:Show(cur_mercenary, temp_mercenary)
    ...
    if template_info.have_artifact then
        if temp_mercenary.is_open_artifact then
            if temp_mercenary.artifact_lv == nil  then
                temp_mercenary.artifact_lv = 1
            end
            if temp_mercenary.artifact_lv > 0 and template_info.have_artifact_upgrade then
                after_open_value = temp_mercenary.artifact_lv
                self.after_artifact_text:setString(lang_constants:Get("mercenary_swarp_property_grade_text")..temp_mercenary.artifact_lv)
            else
        ...
        if cur_mercenary.is_open_artifact then
            if cur_mercenary.artifact_lv == nil  then
                cur_mercenary.artifact_lv = 1
            end

            if cur_mercenary.artifact_lv > 0 and template_info.have_artifact_upgrade then
                cur_open_value = cur_mercenary.artifact_lv
                self.cur_artifact_text:setString(lang_constants:Get("mercenary_swarp_property_grade_text")..cur_mercenary.artifact_lv)
            else
...
function transmigration_panel:Show(mode, cur_mercenary)
    ...
    if (conf and not conf.mult_num2) or free_tag then
...
function transmigration_panel:SetAfterMercenaryInfo(mercenary)
    if mercenary then
        temp_mercenary.template_id = mercenary.template_id
        ...
        temp_mercenary.artifact_lv = mercenary.artifact_lv
    else
        temp_mercenary.template_id = 0
