...
function formation_recommend_panel:CheckRecommendLogic(recommend_type)
    ...
    local list_mercenary_num = {}
    if mercenary_number <= need_capacity then 
        panel_util:SortMercenary(SORT_TYPE["recommend_by_skill"], list)

        local remove_table = {}
        for key, mercenary in pairs(list) do 
            local status1, status2 = troop_logic:IsMercenaryInMineFormation(mercenary)
            local template_id = mercenary.template_id
            local num_limit = config_manager.mercenary_config[template_id]["num_limit"]
            
            list_mercenary_num[template_id] = list_mercenary_num[template_id] or 0
            if (troop_logic:IsMineFormation(self.formation_id) and status1) or (num_limit and num_limit > 0 and list_mercenary_num[template_id] >= num_limit) then
                table.insert(remove_table, key)
            else
                list_mercenary_num[template_id] = list_mercenary_num[template_id] + 1
            end
        end

        table.sort( remove_table, function ( a,b ) return a > b end )

        for _,key in ipairs(remove_table) do
        ...
        for _, mercenary in pairs(list) do 
            local status1, status2 = troop_logic:IsMercenaryInMineFormation(mercenary)
            if troop_logic:IsMineFormation(self.formation_id) and status1 then
        ...
        for step = 1, 3 do 
            if #list < need_capacity then 
                ...
                for _, mercenary in ipairs(commands_table[step]["to_list"]) do 
                    ...
                    local num_limit = config_manager.mercenary_config[template_id]["num_limit"]
                    if not num_limit or num_limit == 0 or list_mercenary_num[template_id] < num_limit then
                        if step == 1 then 
