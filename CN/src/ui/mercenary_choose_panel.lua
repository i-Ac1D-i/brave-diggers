...
function mercenary_choose_panel:Show(mode, src_instance_id, src_position, formation_id)
    ...
    if self.mode == CHOOSE_SHOW_MODE["evolution"] then
        ...
        if self.mode == CHOOSE_SHOW_MODE["formation"] then
            graphic:DispatchEvent("update_battle_point", troop_logic:GetTroopBP(formation_id))
...
function mercenary_choose_panel:CopyMercenaryList()
    ...
    for id, mercenary in pairs(mercenary_list) do
        if self.mode == CHOOSE_SHOW_MODE["formation"] then
            if not troop_logic:IsMercenaryInFormation(mercenary, self.formation_id) then
...
function mercenary_choose_panel:ShowSortResult(sort_type, sort_range)
    ...
    for k,v in pairs(constants["MINE_TROOP_ID"]) do
        if v == self.formation_id then
            is_mine_formation = true
            break
        end
    end

    if self.mode == CHOOSE_SHOW_MODE["material"] or self.mode == CHOOSE_SHOW_MODE["acceptor"] then
        is_mine_formation = true
    end


    panel_util:SortMercenary(self.sort_type, self.mercenary_list, is_mine_formation)
...
function mercenary_choose_panel:ShowSingleRowMercenary(mercenary_row, sub_panel_row, pos_y)
    for col = 1, 5 do
        ...
        if tag <= self.mercenary_num then
            local is_mine_formation = false
            --判断是否是矿山阵容选择
            for k,_id in pairs(constants["MINE_TROOP_ID"]) do
                if self.formation_id == _id then
                    is_mine_formation = true
                    break
                end
            end
            ...
            sub_panel:Load(mercenary, self.sort_type, is_mine_formation)

            if self.formation_id == constants["GUILD_WAR_TROOP_ID"] then 
                sub_panel:SetRoleTipVisible(false)
            end
            
            if self.mode == CHOOSE_SHOW_MODE["evolution"] then
                local is_in_formation = mercenary.formation_info ~= 0
                sub_panel.fight_img:setVisible(is_in_formation)
...
function mercenary_choose_panel:RegisterWidgetEvent()
    ...
    self.view_mercenary_method = function(widget, event_type)
        if event_type == ccui.TouchEventType.ended then
            ...
            if self.mode == CHOOSE_SHOW_MODE["formation"] then
                ...
                local mine_status1, mine_status2 = troop_logic:IsMercenaryInMineFormation(mercenary)   --矿山的状态
                    --是否在矿山阵容中
                if not mine_status2 and troop_logic:IsMineFormation(self.formation_id) then
                    --该佣兵正在进行开采，无法进行调整
                    graphic:DispatchEvent("show_prompt_panel", "mercenary_is_mine_now_tips")
                    return 
                end
            ...
            elseif self.mode == CHOOSE_SHOW_MODE["evolution"] then
                ...
                if mine_status2 then
                    if mine_status1 then
                         --在矿山阵容中
                        graphic:DispatchEvent("show_prompt_panel", "mercenary_transmigrate_in_mine_formation_tips")
                        return
                    end
                else
                    --该佣兵正在进行开采，无法进行调整
                    graphic:DispatchEvent("show_prompt_panel", "mercenary_transmigrate_is_mining_tips")
                    return 
                end
    ...
    self.back_btn:addTouchEventListener(function(widget, event_type)
        if event_type == ccui.TouchEventType.ended then
            ...
            elseif self.mode == CHOOSE_SHOW_MODE["formation"] then
                graphic:DispatchEvent("update_battle_point", troop_logic:GetTroopBP(self.formation_id))
            end
        end
    end)
