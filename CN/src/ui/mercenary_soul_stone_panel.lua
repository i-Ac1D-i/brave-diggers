...
function mercenary_soul_stone_panel:Init()
    ...
    --TAG:MASTER_MERGE
    if feature_config:IsFeatureOpen("contract_soul_bone") then
        --魂骨
    ...
    local num = #client_constants.CRAFT_COST_RESOURCE
    for i = 1, num do
        ...
        if i <= 5 then
            ...
            sub_panel.root_node:setPosition(first_row_begin_x + (i - 1 - 5) * internal_x, second_row_begin_y)
        end
    ...
    self.cut_down_time_desc = self.root_node:getChildByName("desc1")
    self.cut_down_time_text = self.root_node:getChildByName("time")
    self.cut_down_time_icon = self.root_node:getChildByName("diamond")
    self.cut_time = 0
...
function mercenary_soul_stone_panel:UpdateTabStatus(tab_type)
    ...
    elseif tab_type == TAB_TYPE["craft"] then
        ...
        if not platform_manager:GetChannelInfo().mercenary_contract_use_soul_bone then
            self:LoadCraftInfo()
        end
    end
end

function mercenary_soul_stone_panel:Update(elapsed_time)
    if self.cut_time > 0 then
        self.cut_time = self.cut_time - elapsed_time
        if self.cut_down_time_text then
            local time_arry = utils:getTimeString(self.cut_time,true)
            local h = time_arry.hours or 0
            local m = time_arry.minutes or 0
            local s = time_arry.seconds or 0
           self.cut_down_time_text:setString(string.format("%02d:%02d:%02d",h,m,s)) 
        end
    elseif self.cut_down_time_desc then  
        if self.cut_down_time_desc:isVisible() then
            self:UpdateTabStatus(TAB_TYPE["craft"])
        end
    end
end
...
function mercenary_soul_stone_panel:LoadRecruitInfo()
    ...
    if self.cut_time > 0 then
        self.confirm_btn:setTitleText(lang_constants:Get("cut_time_btn_desc"))
    ...
    if platform_manager:GetChannelInfo().mercenary_contract_use_soul_bone then
        --魂骨
...
function mercenary_soul_stone_panel:LoadCraftInfo()
    --是否在召唤时间内
    local cut_down_time = troop_logic:GetMercenaryRecruitTime(self.template_id)
    if cut_down_time > 0 then
        local now_time = time_logic:Now()
        self.cut_time = math.floor(cut_down_time -  now_time)
        if self.cut_time > 0 then
            self:ShowMercenaryCutDownTime()
        end
    else
        self.cut_time = 0
    end
    ...
    local count = troop_logic:GetMercenaryLibraryCount(template_info.ID) or 0
...
function mercenary_soul_stone_panel:ShowMercenaryCutDownTime()
    if self.bg_img then
        self.bg_img:setContentSize(cc.size(self.bg_img:getContentSize().width, 610))
    end
    if self.cut_down_time_desc then
        self.cut_down_time_desc:setVisible(true)
    end
    if self.cut_down_time_text then
        self.cut_down_time_text:setVisible(true)
    end
    if self.cut_down_time_icon then
        self.cut_down_time_icon:setVisible(true)
    end
end

function mercenary_soul_stone_panel:HideMercenaryCutDownTime()
    if self.bg_img then
        self.bg_img:setContentSize(cc.size(self.bg_img:getContentSize().width, 579))
    end
    if self.cut_down_time_desc then
        self.cut_down_time_desc:setVisible(false)
    end
    if self.cut_down_time_text then
        self.cut_down_time_text:setVisible(false)
    end
    if self.cut_down_time_icon then
        self.cut_down_time_icon:setVisible(false)
    end
end
...
function mercenary_soul_stone_panel:RegisterWidget()
    ...
    graphic:RegisterEvent("clear_recurit_library_time_success", function()
    ...
    self.confirm_btn:addTouchEventListener(function(widget, event_type)
        if event_type == ccui.TouchEventType.ended then
            ...
            if self.cur_tab_type ~= TAB_TYPE["recruit"] then
                if self.cut_time > 0 then
                    local mode = client_constants["CONFIRM_MSGBOX_MODE"]["clear_call_mercenary_tips"]
                    graphic:DispatchEvent("show_world_sub_panel", "confirm_msgbox", mode, self.cut_time, self.template_id)
                else
