...
function campaign_event_msgbox:UpdateCostNode()
    ...
    elseif self.mode == CAMPAIGN_MSGBOX_MODE["guild_boss"] then
        self.cost_node[2]:Hide()
        local desc_str,count,cost = guild_logic:GetNeedCostString()
        self.cost_node[1]:LoadIcon(client_constants["CAMPAIGN_RESOURCE_ICON"]["boss_tick"], desc_str)
        self.cost_node[1]:SetTextColor(self:CheckResource(count, cost))
    else
        self.cost_node[2]:Hide()
...
function campaign_event_msgbox:UpdateRewardList()
    ...
    elseif self.mode == CAMPAIGN_MSGBOX_MODE["guild_boss"] then
        -- 奖励列表
        local reward_data = guild_logic:GetNowCurRewardList()
        if reward_data then
            for k,v in pairs(reward_data) do
                local reward_node = campaign_icon_node.New()
                reward_node:Init(self.reward_list, k, false)
...
function campaign_event_msgbox:ShowGuildBossEvent()
    local cave_data = self.event_data
    local title_name = cave_data.name
    local cave_desc = cave_data.desc
    ...
    self:LoadEnemyInfo(cave_data.master_id, lang_constants:Get("campaign_event_enemy_skill"))
...
function campaign_event_msgbox:Show(mode, data)
    ...
    elseif self.mode == CAMPAIGN_MSGBOX_MODE["guild_boss"] then 
        self.event_data = data
        self.boss_id = data.ID
        self:ShowGuildBossEvent()
    end
end
...
function campaign_event_msgbox:RegisterWidgetEvent()
    ...
    self.battle_btn:addTouchEventListener(function (widget, event_type)
        if event_type == ccui.TouchEventType.ended then
            ...
            elseif self.mode == CAMPAIGN_MSGBOX_MODE["guild_boss"] then
                local guild_ticket = resource_logic:GetResourceNum(RESOURCE_TYPE["guild_boss_ticket"])
                if guild_ticket > 0 then
                    guild_logic:GuildBossChallenge(self.boss_id)
                else
                    graphic:DispatchEvent("show_simple_msgbox", lang_constants:Get("msgbox_general_title"),
                     lang_constants:Get("guild_tickets_insufficient"),
    ...
    self.formation_btn:addTouchEventListener(function (widget, event_type)
        ...
        if event_type == ccui.TouchEventType.ended then
            ...
            if self.mode == CAMPAIGN_MSGBOX_MODE["boss_cave"] or self.mode == CAMPAIGN_MSGBOX_MODE["normal_cave"] or self.mode == CAMPAIGN_MSGBOX_MODE["guild_boss"] then 
                local ex_params = { self.mode, self.event_data }
                graphic:DispatchEvent("show_world_sub_scene", "formation_sub_scene", trans_type, mode, back_panel, ex_params)   
            else
