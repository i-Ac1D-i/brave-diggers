...
function rune_sub_panel:Init(root_node)
    ...
    self.can_select_by_exchange = true
...
function rune_sub_panel:Show(data_index, rune_info, cell_type)
...
function rune_sub_panel:Hide(data_index)
    self.data_index = data_index

    self.root_node:setVisible(false)
end

function rune_sub_panel:CannotSelect()
    self.can_select_by_exchange = false
    self.root_node:setColor(panel_util:GetColor4B(0x7F7F7F))
    self.bg_img:setColor(panel_util:GetColor4B(0x7F7F7F))
    self.icon_img:setColor(panel_util:GetColor4B(0x7F7F7F))
    self.level_text:setColor(panel_util:GetColor4B(0x7F7F7F))
    self.top_quality_icon:setColor(panel_util:GetColor4B(0x7F7F7F))
    self.equipped_icon:setColor(panel_util:GetColor4B(0x7F7F7F))
end

function rune_sub_panel:CanSelect()
    self.can_select_by_exchange = true
    ...
    self.level_text:setColor(panel_util:GetColor4B(0xffffff))
    self.top_quality_icon:setColor(panel_util:GetColor4B(0xffffff))
    self.equipped_icon:setColor(panel_util:GetColor4B(0xffffff))
end
...
function rune_bag_panel:Show(show_type, select_list, target_rune_id, select_rune1_info)
    self.show_type = show_type
    self.target_rune_id = nil
    self.select_rune1_info = nil
    ...
    elseif self.show_type == client_constants["RUNE_BAG_SHOW_TYPE"]["EXCHANGE"] then
        self.select_index = target_rune_id
        self.select_rune1_info = select_rune1_info
        self.rune_list = rune_logic:GetExchangeRuneList(select_rune1_info)
    else
...
function rune_bag_panel:ResetAllBagCell(select_list)
    for rune_id,rune_info in pairs(self.select_list) do
        for _,sub_row_panel in ipairs(self.rune_sub_panels) do
            for _,sub_panel in ipairs(sub_row_panel) do
                if sub_panel.rune_info and sub_panel.rune_info.rune_id == rune_id then
                    sub_panel.is_selected = false
                    sub_panel.select_img:setVisible(false)
                end
            end
        end
    end
...
function rune_bag_panel:RefreshRuneCells(offset_y)
    for _,sub_row_panel in ipairs(self.rune_sub_panels) do
        for _,sub_panel in ipairs(sub_row_panel) do
            if sub_panel.data_index <= #self.rune_list then
            elseif sub_panel.data_index <= rune_logic:GetRuneBagCapacity() then
                sub_panel:Show(sub_panel.data_index, nil, RUNE_BAG_CELL_STATUS["no_rune"])
            elseif sub_panel.data_index == (rune_logic:GetRuneBagCapacity() + 1) then
                sub_panel:Show(sub_panel.data_index, nil, RUNE_BAG_CELL_STATUS["lock"])
            else
                sub_panel:Hide(sub_panel.data_index)
            end
            sub_panel.root_node:setPositionY(sub_panel.root_node:getPositionY() + offset_y)
        end
    end
end

function rune_bag_panel:RefreshRuneBag()
    local bag_cell_num = rune_logic:GetRuneBagCapacity()
    local row_num = 0
    if self.show_type == client_constants["RUNE_BAG_SHOW_TYPE"]["PACKAGE"] and bag_cell_num < rune_logic:GetMaxRuneBagCapacity() then
        row_num = math.ceil((bag_cell_num + 1) / BAG_CELL_COL)
    ...
    if row_num > self.row_num then
        --本次新增的多少行
        local offset_num = row_num - self.row_num
        self.row_num = row_num

        local height = math.max(self.row_num * SUB_PANEL_HEIGHT, self.reuse_scrollview.sview_height)
        if self:CreateRuneBag(self.row_num) then
            local begin_y = height + FIRST_SUB_PANEL_OFFSET
            for rune_row = 1, self.sub_panel_row do
                self:ShowSingleRowRune(rune_row, self.rune_sub_panels[rune_row], begin_y - (rune_row - 1) * SUB_PANEL_HEIGHT)
            end
            ...
            self.scroll_view:setInnerContainerSize(cc.size(self.reuse_scrollview.sview_width, height))
            self:RefreshRuneCells(SUB_PANEL_HEIGHT * offset_num) -- 设置偏移量
        end
    else
        self:RefreshRuneCells(0)
    end
end
...
function rune_bag_panel:ShowSingleRowRune(rune_row, sub_row_panel, pos_y)
    for col = 1, BAG_CELL_COL do
        ...
        if data_index <= #self.rune_list then
            sub_panel:Show(data_index, self.rune_list[data_index], RUNE_BAG_CELL_STATUS["have_rune"])
            if self.select_rune1_info then
                if self.select_rune1_info.rune_id == self.rune_list[data_index].rune_id then
                    sub_panel:CannotSelect()
                elseif self.select_rune1_info.template_info.quality ~= self.rune_list[data_index].template_info.quality then
                    sub_panel:CannotSelect()
                end
            end
        ...
        elseif data_index <= rune_logic:GetRuneBagCapacity() then
            sub_panel:Show(data_index, nil, RUNE_BAG_CELL_STATUS["no_rune"])
        elseif data_index == (rune_logic:GetRuneBagCapacity() + 1) then
            sub_panel:Show(data_index, nil, RUNE_BAG_CELL_STATUS["lock"])
        else
            sub_panel:Hide(data_index)
        end
...
function rune_bag_panel:SelectRune(sub_panel)
    if sub_panel then
        ...
        if rune_info and cell_type == RUNE_BAG_CELL_STATUS["have_rune"] then
            if self.show_type == client_constants["RUNE_BAG_SHOW_TYPE"]["EXCHANGE"] then
                if not sub_panel.can_select_by_exchange then
                    graphic:DispatchEvent("show_prompt_panel", "rune_only_select_same_quality")
                    return 
                end
            end
            if self.show_type == client_constants["RUNE_BAG_SHOW_TYPE"]["SELECT_MUILT"] then
                ...
                self.select_list[rune_info.rune_id] = sub_panel.is_selected and sub_panel.rune_info or nil
                ...
                sub_panel:ShowSelectImg(true)
                self.select_list[rune_info.rune_id] = sub_panel.rune_info
                self.select_num = 1
            end
        ...
        elseif cell_type == RUNE_BAG_CELL_STATUS["lock"] then
            if rune_logic:IsCanBuyRuneBugCell(true) then
                local mode = client_constants.BATCH_MSGBOX_MODE["rune_bag_numbers"]
...
function rune_bag_panel:RegisterWidgetEvent()
    ...
    self.confirm_btn:addTouchEventListener(function(widget, event_type)
        if event_type == ccui.TouchEventType.ended then
            ...
            for rune_id,rune_info in pairs(self.select_list) do
                if rune_info then
                    table.insert(select_rune_list, rune_info)
                end
            end
            ...
            if #select_rune_list > 0 then
                ...
                if self.show_type == client_constants["RUNE_BAG_SHOW_TYPE"]["EXCHANGE"] then
                    graphic:DispatchEvent("rune_bag_select_confirm", self.show_type, select_rune_list, self.select_index)
                else
