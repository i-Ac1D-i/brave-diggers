...
local escort_rule_panel = panel_prototype.New(true)
function escort_rule_panel:Init(root_node)
    self.root_node = cc.CSLoader:createNode("ui/tramcar_rule_msgbox.csb")
...
function escort_rule_panel:makeUpView()
    local offsetY = 25;
    local now_y = 0
    local scrollview = self.root_node:getChildByName("scrol_view")
    local text_6 = scrollview:getChildByName("Text_2_0_0_0_0_0")
    local text_6_height = self:getLines(text_6:getString(),18*3) * text_6:getVirtualRendererSize().height
    text_6:setContentSize({width = 436, height = text_6_height })
    now_y = now_y + text_6_height+offsetY-10
    text_6:setPositionY(now_y)

    local text_5 = scrollview:getChildByName("Text_2_0_0_0_0")
    ...
    local text_4 = scrollview:getChildByName("Text_2_0_0_0")
    ...
    local text_3 = scrollview:getChildByName("Text_2_0_0")
    ...
    local text_2 = scrollview:getChildByName("Text_2_0")
    ...
    local text_1 = scrollview:getChildByName("Text_2")
...
function escort_rule_panel:getLines(str,size)
   return math.ceil(string.len(str)/size) 
end

function escort_rule_panel:Show()
    self.root_node:setVisible(true)
end

function escort_rule_panel:RegisterWidgetEvent()
    panel_util:RegisterCloseMsgbox(self.root_node:getChildByName("close_btn"), self:GetName())
end

return escort_rule_panel
