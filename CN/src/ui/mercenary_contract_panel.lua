...
function mercenary_contract_panel:Init()
    ...
    if channel.meta_channel == "txwy_dny" then
        --东南亚描述文字过多没有显示完全
        local choose_target_node_desc_text = self.choose_target_node:getChildByName("desc_0")
        choose_target_node_desc_text:setContentSize(cc.size(choose_target_node_desc_text:getContentSize().width, choose_target_node_desc_text:getContentSize().height + 10))
    end
...
function mercenary_contract_panel:UpdateLeaderContractInfo()
    ...
    if self.conf_index == #leader_contract_config then
        self.leader_contract_exp_text:setString(lang_constants:Get("level_max"))
...
function mercenary_contract_panel:RegisterWidgetEvent()
    ...
    self.spine_node:registerSpineEventHandler(function(event)
        ...
        if event.animation == "contract_first_enter" then
