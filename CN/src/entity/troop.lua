...
function troop:SetProperty(max_bp, cur_bp, cur_shield_bp, enemy, speed, defense, dodge, authority, original_speed, original_defense, original_dodge, original_authority)
    ...
    self.cur_bp = cur_bp
    self.max_bp = max_bp
    self.cur_shield_bp = cur_shield_bp or 0
...
function troop:SetCultivation(cultivation) 
    self.cultivation_property = cultivation
end

function troop:SetWeaponInfo(weapon_info) 
...
function troop:CalcDamageFactor()
    ...
    self.dodge_chance = math.ceil(self.dodge / (100 + self.dodge) * 1000)
