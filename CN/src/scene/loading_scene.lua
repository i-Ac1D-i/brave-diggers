...
local IMG_TASK_LIST =
{
    ...
    { "ui/title.png", "ui/title.plist" },
    { "ui/recover.png", "ui/recover.plist" },
}
...
function loading_scene:ctor(next_scene_name)
    ...
    self:registerScriptHandler(function(event)
        if event == "enter" then
            ...
            --TAG:MASTER_MERGE
            self.spine_node:setSkin(tostring(math.random(1, client_constants["LOADING_SPINE_SKIN_NUM"])))
            ...
            --TAG:MASTER_MERGE
            if platform_manager:GetChannelInfo().update_img_task_list then
                self:UpdateImgTaskListWithLanguage()
            else
                table.insert(IMG_TASK_LIST, { "role/mercenary.png", "role/mercenary.plist" })
                table.insert(IMG_TASK_LIST, { "role/artifact.png", "role/artifact.plist" })
            end
