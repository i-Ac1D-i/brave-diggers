...
local JUMP_PANEL_ZORDER = 1399   --  跳转的提示界面
local BLOOD_REPLACE_ZORDER = 1399   --  血钻替代材料界面
...
function world_scene:ctor()
    self:registerScriptHandler(function(event)
        if event == "enter" then
            ...
            -- 资源跳转界面 
            self.jump_panel = require "ui.jump_panel" 
            self.jump_panel:Init()
            self:addChild(self.jump_panel:GetRootNode(), JUMP_PANEL_ZORDER) 
            --血钻替代
            self.blood_replace_panel = require "ui.blood_replace_panel" 
            self.blood_replace_panel:Init()
            self:addChild(self.blood_replace_panel:GetRootNode(), BLOOD_REPLACE_ZORDER) 
...
function world_scene:OnEnterScene()
	...
	elseif notice_logic:HasNewNotice() then

        elseif reminder_logic:IsShowGuildWarNotify() then
            graphic:DispatchEvent("show_world_sub_panel", "notification_msgbox", 2)
...
function world_scene:RegisterEvent()
    -- 资源跳转
    graphic:RegisterEvent("show_mask_node", function()
            if self.mask_node:isVisible() then
                self.open_from_jum = nil  --如果mask_node已经处于显示状态,则标记下
            else
                self.mask_node:setVisible(true)
                self.open_from_jum = true  
            end
        end)
    graphic:RegisterEvent("hide_mask_node", function()
            if self.open_from_jum then
                self.mask_node:setVisible(false)
                self.open_from_jum = nil
            end
    end)
    ...
    local hide_world_sub_panel = function(panel_name, ...)
        ...
        --160926-WYJ:击过快时遮罩层层级还未在下一帧重新排序，会造成卡死，在此次直接手动调用结点排序方法
        self:sortAllChildren()
    ...
    -- 资源跳转
    graphic:RegisterEvent("show_jump_panel", function(...)
        if scene_manager:GetCurrentSceneName() == "world" then
            self.jump_panel:Show(...) 
        end
    end)

    graphic:RegisterEvent("hide_jump_panel", function(...)
        if scene_manager:GetCurrentSceneName() == "world" then
            self.jump_panel:GetRootNode():setVisible(false) 
            graphic:DispatchEvent("hide_mask_node")  
        end
    end)

    --血钻替换
    graphic:RegisterEvent("show_blood_replace_panel", function(...)
        if scene_manager:GetCurrentSceneName() == "world" then
            self.blood_replace_panel:Show(...) 
        end
    end)

    graphic:RegisterEvent("hide_blood_replace_panel", function(...)
        if scene_manager:GetCurrentSceneName() == "world" then
            self.blood_replace_panel:GetRootNode():setVisible(false) 
        end
    end)
    ...
    graphic:RegisterEvent("show_simple_msgbox", function(title, desc, confirm_txt, close_txt, callback, close_callback, close_callback2)
        if self.jump_panel.root_node:isVisible() then  --  屏蔽原来的弹窗
            return
        end
        self.msgbox.root_node:setLocalZOrder(ALERT_PANEL_ZORDER)
    ...
    graphic:RegisterEvent("show_new_notice", function()
        ...
        if channel.notice_url then
            ...
            graphic:DispatchEvent("show_world_sub_panel", "notice_panel", client_constants["NOTICE_PANEL_MODE"]["notice"])
        end
    end)
