        ...
        if recv_msg.result == "success" then
            if event_id == self.cur_maze_template_info.event_id then
                ...
                if cur_maze_conf.type == 5 then
                    ...
                    -- 解锁新难度时，显示提示
                    for k,v in pairs(config_manager.adventure_income_config) do
                        if v.area_id == self.cur_area_id and v.difficulty == self.cur_difficulty + 1 then
                            graphic:DispatchEvent("show_difficulty_tips")
                            break
                        end
                    end
                end
            ...
            if recv_msg.battle_record then
                ...
                if battle_type == client_constants.BATTLE_TYPE["vs_monster"] then
                    if recv_msg.result == "success" then
                        ...
                        daily_logic:LivenessOpen(self.cur_maze_id)
                        if TalkingDataGA then
