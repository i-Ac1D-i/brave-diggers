...
function sns:CanShare(event_type, param1)
    ...
    elseif event_type == SNS_EVENT_TYPE["share_achievement"] then
        ...
        if self.achievement_share_state[param1] == 1 or achievement_logic:GetCurStep(param1) ~= #config_manager.achievement_config[param1] then
            return false
        end
...
function sns:GetAllGameRequests()
    ...
    if channel_info.meta_channel == "txwy" or channel_info.meta_channel == "txwy_dny" then
         platform_manager:DispatchEvent("sns_query_game_request_result",0,{inviter_list = "",sns_platform = channel_info.meta_channel,sns_uid = user_login.openid})  
