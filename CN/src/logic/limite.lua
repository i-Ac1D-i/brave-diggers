...
function limite:Init()
    ...
    self.is_open_blood_buy = false  -- 是否开启血钻购买活动
    self.find_carnival = false
...
function limite:GetLimites()
    ...
    if self.limites == nil or #self.limites <= 0 then
        self.limites = payment_logic:GetProductsInfoByType(5)  
    end
...
--是否开启了用血钻购买
function limite:IsOpenBloodBuy()
    local config = carnival_logic:GetSpecialCarnival(CARNIVAL_TEMPLATE_TYPE["limite_package"]) 
    if config and config.carnival_type == CARNIVAL_TYPE["limite_pacakge_blood"] then
...
--使用血钻购买活动信息
function limite:GetBloodBuyConfig()
    local config = carnival_logic:GetSpecialCarnival(CARNIVAL_TEMPLATE_TYPE["limite_package"]) 
    if config and config.carnival_type == CARNIVAL_TYPE["limite_pacakge_blood"] then
        local info = carnival_logic.info_map[config.key]
        local index = 1
        local find = false
        if info and info.step_reward then
            for k,v in pairs(info.step_reward) do
                index = k
                if v ~= 0 then
                    find = true
                    break
...
--使用血钻购买
function limite:BuyLimiteByBlood()
    if self.is_open_blood_buy then
        local config = self:GetBloodBuyConfig()
        if config then
            network:Send({take_carnival_reward = {key = config.key, step = self.show_index }} )
        end
    end
end
