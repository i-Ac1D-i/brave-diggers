...
function reward:ResetRewardInfoList()
    ...
    network:RegisterEvent("reward_info_list", function(recv_msg)
        ...
        for i, reward_info in ipairs(recv_msg.reward_info_list) do
            ...
            elseif reward_type == REWARD_TYPE["mercenary"] then
                --佣兵
                local mercenary = troop_logic:CreateMercenary(reward_info.param1, reward_info.param2, reward_info.param3, reward_info.mercenary_att)
                reward_info["mercenary_id"] = mercenary.instance_id
                local quality = mercenary.template_info.quality
                if quality == client_constants["QUALITY_TYPE"].quality4 then
                    achievement_logic:UpdateStatisticValue(constants.ACHIEVEMENT_TYPE["mercenary_quality4"], 1)
                elseif quality == client_constants["QUALITY_TYPE"].quality5 then
                    achievement_logic:UpdateStatisticValue(constants.ACHIEVEMENT_TYPE["mercenary_quality5"], 1)
                elseif quality == client_constants["QUALITY_TYPE"].quality6 then
                    achievement_logic:UpdateStatisticValue(constants.ACHIEVEMENT_TYPE["mercenary_quality6"], 1)
                end
            ...
            elseif reward_type == REWARD_TYPE["soul_stone"] then
                troop_logic:UpdateSoulStone(reward_info.param1, reward_info.param2)
        ...
        self.last_reward_source = recv_msg.source
