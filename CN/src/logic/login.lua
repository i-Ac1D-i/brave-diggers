...
local CONVERT_NET = constants["CONVERT_NET"]
local CONVERT_PLAT = constants["CONVERT_PLAT"]
        ...
        if _G["T_SERVER_LIST"] then
            function login:ParseServerList()
                ...
                if content == "" then
                    json_obj = {
                        game_server = {
                            { ip = "127.0.0.1", name = "local", id = 1, status = 1 },
                        }
                    }
...
function login:IsNeedUpdateApp(code_version, remote_version)
    ...
    if string.find(remote_version, ".") then
        return self:IsNeedUpdateApp2(self.origin_version, remote_version)
    end
...
function login:IsNeedUpdateApp2(origin_version, remote_version)
    if not origin_version then
        return false
    end
    ...
    http_client:Get(url, function(status_code, content)
        ...
        if status_code == 200 and string.len(content) ~= 0 then
            ...
            if (auth_name and self:IsInAuthMode(self.origin_version, json_obj[auth_name])) or (auth_code and self:IsInAuthMode2(self.code_version, json_obj[auth_code])) then
                ...
                -- self.server_list[1].ip = "banshu.mu77.aam.mu77.com"
...
function login:ParseChatServerList()
    local url = platform_manager:GetChatServerListUrl()
    if url and url ~= "" then
        http_client:Get(url, function(status_code, content)
            if status_code == 200 and string.len(content) ~= 0 then
                local json_obj = json:decode(content)
                if json_obj.chat_server then
                    self.chat_server_list = json_obj.chat_server
                end
            else
                self.chat_server_list = {}
            end
        end)
    end
end

function login:GetChatServerList()
    return self.chat_server_list or {}
end
...
function login:UserLogin(server_id)
    ...
    if platform_manager:GetChannelInfo().need_device_info and PlatformSDK.getDeviceInfo then
        local str 
        if TARGET_PLATFORM == cc.PLATFORM_OS_WINDOWS or TARGET_PLATFORM == cc.PLATFORM_OS_MAC or TARGET_PLATFORM == cc.PLATFORM_OS_LINUX then
            str = "{\"width\":\"\",\"height\":\"\",\"density\":\"\",\"netState\":\"\",\"macAddress\":\"\", \"IMSI\":\"\", \"deviceId\":\"PC\", \"mode\":\"\", \"osVersion\":\"\"}"
        else
            str = PlatformSDK.getDeviceInfo() 
        end 

        local device_info = json:decode(str) 
        self.device_info = device_info
        net_state = CONVERT_NET[device_info.netState] or -1
        device_platform = CONVERT_PLAT["ios"] 
        if string.find(self.channel_info.name,"_android") then
            device_platform = CONVERT_PLAT["android"] 
        end
...
function login:UploadData(user_id, leader_name)
    local url = platform_manager:GetChannelInfo().upload_device_info
    if url and self.device_info then
        utils:getNetIP(function(ip) 
            local channel_info = platform_manager:GetChannelInfo()
            local accountId = '-1'
            local roleId = user_id
            local roleName = leader_name 
            local level = '1'
            local vipLevel = '1'
            local serverId = configuration:GetServerId()
            local channel = channel_info.signin[1] 
            local platform = channel_info.name 
            local ip = ip
            local version = configuration:GetVersion()
            local netMode = self.device_info.netState 
            local deviceId = self.device_info.deviceId
            local behave = 'deviceactive'
            local para1 = self.device_info.mode
            local para2 = self.device_info.osVersion
            local para3 = self.device_info.netState
            local para4 = self.device_info.IMSI

            self.device_info["accountId"] = accountId
            self.device_info["roleId"] = roleId
            self.device_info["roleName"] = roleName
            self.device_info["level"] = level
            self.device_info["vipLevel"] = vipLevel
            self.device_info["serverId"] = serverId
            self.device_info["channel"] = channel
            self.device_info["platform"]= platform
            self.device_info["ip"] = ip
            self.device_info["version"] = version

            local remarks = json:encode(self.device_info) 

            local post_data = string.format("daemon=datacenter&action=InsertLog&appType=aam&accountId=%s&roleId=%s&roleName=%s&level=%s&vipLevel=%s&serverId=%s&channel=%s&platform=%s&ip=%s&version=%s&netMode=%s&deviceId=%s&behave=%s&remarks=%s",
                                accountId,roleId,roleName,level,vipLevel,serverId,channel,platform,ip,version,netMode,deviceId,behave,remarks)
...
function login:RegisterMsgHandler()
    network:RegisterEvent("login_ret", function(recv_msg)
        ...
        if recv_msg.result == "success" then
            utils:setNetIP(recv_msg.client_ip)
            ...
            if platform_manager:GetChannelInfo().need_device_info and PlatformSDK.loginPlayerYXHY then
                PlatformSDK.loginPlayerYXHY(string.format("%s|%s|%d|%s","mu77",recv_msg.user_id,self.server_id,configuration:GetServerName()or ""));
            end
        elseif recv_msg.result == "create_leader" then
            ...
            if platform_manager:GetChannelInfo().need_device_info and PlatformSDK.registerPlayerYXHY then
                PlatformSDK.registerPlayerYXHY(recv_msg.user_id)  
            end 
