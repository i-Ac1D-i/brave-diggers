...
function payment:TryBuy(product_info)
    ...
    elseif #channel_info.pay == 2 then
        -- 选择微信或支付宝
...
function payment:OnFinishAllQuery()
    ...
    elseif self.channel.enable_query_products then
        local server_id = login_logic.server_id -------------------FYD  添加server_id参数,txwy android要用到
        PlatformSDK.queryProductList(self.raw_products, "",server_id)  --增加一个参数
