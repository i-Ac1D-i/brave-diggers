...
function merchant:Init()
    ...
    self.buy_refresh_times = 1
    self.drak_merchant_refresh_time = 1
...
--交易
function merchant:Exchange(order_id, merchant_type)
    local found = false

    for i = 1, self.order_num[merchant_type] do -- do
        ...
        if self.orders[merchant_type][i].order_id == order_id then
            ...
            merchant_type = self.orders[merchant_type][i].merchant_type
            found = true
            break
        end
    end
...
function merchant:GetResetPrice(merchant_type)
    ...
    if self.drak_merchant_refresh_time > #merchant_ref_config then
        self.drak_merchant_refresh_time  = merchant_ref_config[#merchant_ref_config]["group_id"]
    end

    if self.buy_refresh_times > #merchant_ref_config then
        self.buy_refresh_times = merchant_ref_config[#merchant_ref_config]["group_id"]
    end

    local price = merchant_ref_config[self.drak_merchant_refresh_time]["group_id"]    --黑市
    if merchant_type == constants.MERCHANT_TYPE["WHITE"] then  --神秘商店
        price = merchant_ref_config[self.buy_refresh_times]["group_id"]
    end
...
function merchant:RegisterMsgHandler()
    ...
    network:RegisterEvent("merchant_soul_chip_ret", function(recv_msg)
        ...
        if recv_msg.result == "success" then
            ...
            --使用reward_panel显示额外的奖励
            local source = constants["REWARD_SOURCE"]["merchant_exchange"]
            local reward_info_list = {}
            local reward_info =
            {
                ["id"] = constants.REWARD_TYPE["resource"],
                ["param1"] = constants.RESOURCE_TYPE["soul_chip"],
                ["param2"] = self.extra_soul_chip_num
            }

            table.insert(reward_info_list, reward_info)
            graphic:DispatchEvent("show_world_sub_panel", "reward_panel")
