...
function mining:ResetArea(lv)
    ...
    begin_lv = MINING_BOSS_MAP[CONST_BLOCK_TYPE["seven_doom"]]
    for boss_type, lv in pairs(MINING_BOSS_MAP) do
        if next_lv > 0 and next_lv <= lv and begin_lv >= lv then
            begin_lv = lv
            self.next_boss_id = boss_type
        end
    end
...
--
function mining:RefreshGolemLeve()
...
function mining:GetBlockInfo(block_x, block_y)
    ...
    if block_type < CONST_BLOCK_TYPE["empty"] or block_type > CONST_BLOCK_TYPE["harder_rock"] then
        return
    end

    local block_info = config_manager.mining_dig_info_config[block_type]

    if block_info.destroy_level == 0 then
        return
    end
...
function mining:DigBlock(block_x, block_y)
    ...
    local is_connected = self:CheckConnected(block_x, block_y)
    if is_connected then
        network:Send({ dig_block = {x = block_x, y = block_y} })
        return "success"
    else
...
function mining:SolveEvent(event_id)
    ...
    network:Send({ dig_block = {x = self.cur_event_position.x, y = self.cur_event_position.y} })
    return true
end
...
function mining:ChallengeCaveBoss()
    if self.cave_boss_lv > 0 then 
        ...
        -- local cave_data = self:GetCaveEventData(constants["CAVE_BOSS_EVENT_TYPE"], self.cave_boss_lv)

        -- local genre_table = common_function_util.Split(cave_data.genre_limit, '|')
        -- if troop_logic:CheckGenreLimit(genre_table) then
        --     graphic:DispatchEvent("show_prompt_panel", "mining_cave_genre_limit")
        --     return
        -- end
...
function mining:RegisterMsgHandler()
    ...
    network:RegisterEvent("use_mining_tool_ret", function(recv_msg)
        if recv_msg.result == "success" then
            graphic:DispatchEvent("use_mining_tool", recv_msg.resource_id)
