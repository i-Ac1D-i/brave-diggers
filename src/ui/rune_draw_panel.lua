...
function draw_platform_sub_panel:Init(root_node, index)
    self.root_node = root_node
    self.platform_img = cc.Sprite:create(DRAW_PLATFORM_SPRITE[index])
...
function rune_draw_panel:MoveToDrawPlatform(new_draw_platform_id, is_scroll_to, result, draw_type, new_rune_list)
    ...
    if is_scroll_to then
        ...
        self.spine_node:registerSpineEventHandler(function(event)
                ...
                elseif animation_name == "jamp" or animation_name == "man_appear" then
                    ...
                    self:RefreshCost()
                    self:ShowNewRuneAnimation(result, draw_type, new_rune_list) 
                end
            end, sp.EventType.ANIMATION_COMPLETE)
    else
...
function rune_draw_panel:ShowNewRuneAnimation(result, draw_type, new_rune_list)
    if #new_rune_list > 0 then
        for empty_index = 1, rune_logic:GetRuneTemporaryBagCapacity() do
            ...
            if not self.rune_sub_panels[empty_index].rune_info then
                ...
                self:MoveToDrawPlatform(draw_info.new_platform_id, true, result, draw_type, new_rune_list)
...
function rune_draw_panel:RegisterEvent()
    ...
    graphic:RegisterEvent("new_rune_in_temporary_bag", function(result, draw_type, new_rune_list)
        if not self.root_node:isVisible() then
            return
        end
    ...
    graphic:RegisterEvent("receive_rune_to_bag", function(target_rune_info, exp_rune_id_list, move_rune_id_list)
        if not self.root_node:isVisible() then
            return
        end
    ...
    self.go_btn:addTouchEventListener(function(widget, event_type)
        if event_type == ccui.TouchEventType.ended then
            ...
            elseif self.is_moving then
                ...
                if rune_logic:IsSelectedAutoGo() then
                    rune_logic:DrawRune( "ten_times" )
                else
                    rune_logic:DrawRune( "once" )
                end
            end
        end
    end)
            ...
            elseif self.is_moving then
                ...
                if self.cur_draw_platform_id < 4 then
                    if rune_logic:IsIgnoreGoToArea4Tip() then
                        rune_logic:DrawRune( "go_to_area_4" )
                    else
