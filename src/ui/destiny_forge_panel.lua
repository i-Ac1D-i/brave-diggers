    ...
    if weapon_lv == constants["MAX_DESTINY_WEAPON_LV"] then
        ...
        for i, weapon_id in pairs(weapon_ids) do
            ...
            elseif i == weapon_lv + 1 then
                ...
                self.need_forge_weapon_icon_img:loadTexture(client_constants["DESTINY_WEAPON_IMG"] .. weapon_id .. ".png", PLIST_TYPE)
                break
            end
        end
        ...
        panel_util:LoadCostResourceInfo(config, self.cost_sub_panels, COST_SUB_PANEL_POS_Y, constants["MAX_DESTINY_FORGE_COST_RESOURCE_TYPE"])
    end
