...
function ore_bag_panel:Init()
    ...
    self.tool_sub_panels[5] = crystal_sub_panel

    tnt_sub_panel:Init(self.tool_node:getChildByName("tool5"), RESOURCE_TYPE["tnt"])
    self.tool_sub_panels[6] = tnt_sub_panel
...
function ore_bag_panel:RegisterWidgetEvent()
    ...
    for i = 1, 5 do
        local tool_sub_panel = self.tool_sub_panels[i]
        tool_sub_panel.use1_btn:addTouchEventListener(use_tool_method1)
        tool_sub_panel.use2_btn:addTouchEventListener(use_tool_method2)
    end
...
function ore_bag_panel:RegisterEvent()
    graphic:RegisterEvent("store_buy_success", function(goods_id)
        ...
        elseif goods_info.type == STORE_GOODS_TYPE["resource"] then
            for i = 1, 6 do
                local sub_panel = self.tool_sub_panels[i]
    ...
    graphic:RegisterEvent("use_mining_tool", function(tool_type)
        if not self.root_node:isVisible() then
            return
        end
