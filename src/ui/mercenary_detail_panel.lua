...
function mercenary_info_sub_panel:Init(root_node)
    ...
    --r2修改右对齐
    local all_right=platform_manager:GetChannelInfo().mercenary_info_sub_panel_all_right
    if all_right then
        --右对齐
...
function mercenary_detail_panel:Show(mode, mercenary_id, formation_id)
...
function mercenary_detail_panel:LoadLeaderSkillInfo()
    ...
    if weapon_id ~= 0 then
        ...
        self.artifact_and_destiny_icon_img:loadTexture(client_constants["DESTINY_WEAPON_IMG"] .. weapon_id .. ".png", PLIST_TYPE)
...
function mercenary_detail_panel:ShowMercenarySkillDesc()
    ...
    self:SetSkillInfoBgContentSize()
...
--设定技能描述背景图片的大小
function mercenary_detail_panel:SetSkillInfoBgContentSize()
    ...
    if line_num > 2 then
        height = height + (line_num - 2) * SKILL_DESC_LINE_HEIGHT
    end
    ...
    self.skill_desc_text:setPositionY(height - 62)

    self.skill_info_bg_img:setPositionY(self.skill_imgs[self.skill_type]:getPositionY() + height / 2)
    self.skill_bg_height = height
    --r2 两行显示
     --位置偏移一点点
    if self.channel_info.mercenary_detail_panel_coop_and_artifact_text_pos_x then
        local offset_y = -5
