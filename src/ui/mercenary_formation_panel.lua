...
function mercenary_formation_panel:CreateMercenarySubPanels()
    ...
    for i = 1, MAX_FORMATION_CAPACITY do
        ...
        self.sub_panels_position[i] = { x = 0, y = 0 }
    ...
    self.sub_panel_height = self.mercenary_sub_panels[1][1].root_node:getContentSize().height
    self.sub_panel_width = self.mercenary_sub_panels[1][1].root_node:getContentSize().width
...
function mercenary_formation_panel:UpdateSkillCoverPercent(formation_id)
    ...
    for i = 1, #formation do
        ...
        if mercenary.is_leader then 
            ...
            if weapon_id ~= 0 then
               local leader_skill = destiny_skill_config[weapon_id]["skill_id"]
...
function mercenary_formation_panel:ResetChooseTracker()
    ...
    self.mercenary_sub_panels[self.cur_group_index][self.cur_mercenary_pos].root_node:setScale(1, 1)
    self.mercenary_sub_panels[self.cur_group_index][self.last_mercenary_pos].root_node:setScale(1, 1)
...
function mercenary_formation_panel:SetArrangeNodeStatus()
    ...
    self.touch_listener:setEnabled(not arrange_isvisible)
...
function mercenary_formation_panel:UnchosenMercenaryAnimation()
    if self.last_mercenary_pos ~= self.cur_mercenary_pos then
        local x, y = self.sub_panels_position[self.last_mercenary_pos].x,  self.sub_panels_position[self.last_mercenary_pos].y
...
function mercenary_formation_panel:ChooseMercenaryAnimation()
    if self.cur_mercenary_pos <= self.cur_mercenary_num then
        local x, y = self.sub_panels_position[self.cur_mercenary_pos].x,  self.sub_panels_position[self.cur_mercenary_pos].y
...
function mercenary_formation_panel:Show(mode)
    ...
    elseif self.mode == client_constants["FORMATION_PANEL_MODE"]["quest"] then
...
--加载整个阵容中的信息
function mercenary_formation_panel:LoadFormationInfo(is_temp_formation, formation_id, group_index)
    formation_id = formation_id or self.formation_id
    group_index = group_index or self.cur_group_index

    self:LoadMercenaryInfo(is_temp_formation, formation_id, group_index)
    ...
    self.formation_tip_text:setString(string.format(lang_constants:Get("mercenary_cur_formation_id"), formation_id))
...
function mercenary_formation_panel:LoadMercenaryInfo(is_temp_formation, formation_id, group_index)
    ...
    for i = 1, self.cur_mercenary_num do
        ...
        if is_temp_formation then
            ...
            --防止拖动后位置错乱
            self.shaking_spine_trackers[i]:Bind(self.sub_panels_position[i].x, self.sub_panels_position[i].y, sub_panel.root_node)
        else
...
function mercenary_formation_panel:ResetMercenarySubPanelPos()
    ...
    for i = 1, constants["MAX_FORMATION_CAPACITY"] do
        local root_node = self.mercenary_sub_panels[self.cur_group_index][i].root_node
        ...
        self.sub_panels_position[i].x = x
        self.sub_panels_position[i].y = y
    end
end
...
function mercenary_formation_panel:ShakingMercenarySubPanels()
    ...
    for i = 1, constants["MAX_FORMATION_CAPACITY"] do
        ...
        local ref_node = self.mercenary_sub_panels[self.cur_group_index][i].root_node
        if i <= self.cur_mercenary_num then
            if shaking then
                tracker:Bind(self.sub_panels_position[i].x, self.sub_panels_position[i].y, ref_node)
            else
                tracker:StopShaking()
            end
        end
...
function mercenary_formation_panel:ResetPosAfterDrag()
    ...
    elseif self.last_find_pos < self.find_pos then
        for i = (self.last_find_pos + 1), self.find_pos do
            local sub_panel = self.mercenary_sub_panels[self.cur_group_index][i]
            local x, y = self.sub_panels_position[i - 1].x, self.sub_panels_position[i - 1].y
    ...
    local last_find_sub_panel = self.mercenary_sub_panels[self.cur_group_index][self.last_find_pos]
    table.remove(self.mercenary_sub_panels[self.cur_group_index], self.last_find_pos)
    table.insert(self.mercenary_sub_panels[self.cur_group_index], self.find_pos, last_find_sub_panel)
...
function mercenary_formation_panel:FindMovingPanelPos()
    ...
    --再找列
    for i = 1, 5 do
        local col_pos_x = self.sub_panels_position[i].x
...
function mercenary_formation_panel:FindMovingPanelEndPos()
    ...
    if self.find_pos ~= self.cur_mercenary_pos then
        ...
        local x, y = self.sub_panels_position[self.find_pos].x, self.sub_panels_position[self.find_pos].y
...
--流派面板滑动
function mercenary_formation_panel:SetMoveLineupBg(move_flag)
...
function mercenary_formation_panel:ChangeFormationTransition(elapsed_time)
    ...
    if self.change_formation_transition_flag then
        ...
        if self.change_formation_transition_time <= TRANSITION_TIME then
            ...
            for i = 1, MAX_FORMATION_CAPACITY do
                local cur_sub_panel = self.mercenary_sub_panels[self.cur_group_index][i]
                cur_sub_panel.root_node:setOpacity(255 * (1 - percent))

                local next_sub_panel = self.mercenary_sub_panels[another_group_index][i]
                next_sub_panel.root_node:setOpacity(255 * percent)

                if self.offset_pos < 0 then
                    next_sub_panel.root_node:setPositionX(self.sub_panels_position[i].x + self.offset_pos + percent * SCREEN_WIDTH)
                    cur_sub_panel.root_node:setPositionX(self.sub_panels_position[i].x + percent * SCREEN_WIDTH)
                else
                    next_sub_panel.root_node:setPositionX(self.sub_panels_position[i].x + self.offset_pos - percent * SCREEN_WIDTH)
                    cur_sub_panel.root_node:setPositionX(self.sub_panels_position[i].x - percent * SCREEN_WIDTH)
                end
            ...
            --切换动画完毕
            if self.finish_change_formatio_transition then
                self.cur_group_index = self:GetAnotherGroupIndex()
                self:ChooseMercenaryAnimation()
                self:SetMultiFormationImgsOpacity()

                --更新属性
                troop_logic.troop_property_changed = true
                ...
                for i = 1, constants["MAX_FORMATION_CAPACITY"] do
                    ...
                    root_node:setPosition(self.sub_panels_position[i].x, self.sub_panels_position[i].y)
                    root_node:setOpacity(255)
                end
                self.finish_change_formatio_transition = false
            end
        end
    end
end
        ...
        if self.change_formation_check_time < 0.2 then
            ...
            if math.abs(distance_x) > 30 and math.abs(distance_x) > distance_y then
                ...
                if next_formation_id ~= self.formation_id then
                    ...
                    local another_group_index = self:GetAnotherGroupIndex()
                    self:LoadFormationInfo(false, next_formation_id, another_group_index)
                    ...
                    self.finish_change_formatio_transition = true
    ...
    graphic:RegisterEvent("update_exploring_merceanry_position", function(mode, index, src_id, dest_id, formation_id)
        ...
        if mode == client_constants["MERCENARY_TO_FORMATION"]["rest"] then
            ...
            if src_pos > 0 and src_pos <= self.cur_mercenary_num then
                ...
                self.mercenary_sub_panels[self.cur_group_index][self.cur_mercenary_num]:Clear(true)
        ...
        elseif mode == client_constants["MERCENARY_TO_FORMATION"]["replace"] then
            ...
            if src_pos > 0 and src_pos <= self.cur_mercenary_num then
                ...
                local sub_panel = self.mercenary_sub_panels[self.cur_group_index][src_pos]
        ...
        elseif mode == client_constants["MERCENARY_TO_FORMATION"]["add"] then
            ...
            local sub_panel = self.mercenary_sub_panels[self.cur_group_index][self.cur_mercenary_pos]
        ...
        elseif mode == client_constants["MERCENARY_TO_FORMATION"]["recommend"] then
            ...
            --local mercenary_num = #troop_logic:GetFormationMercenaryList(self.formation_id)
            for i, sub_panel in pairs(self.mercenary_sub_panels[self.cur_group_index]) do
                if i <= self.cur_mercenary_num then 
                    sub_panel:Clear(true)
...
function mercenary_formation_panel:RegisterWidgetEvent()
            ...
            if self.arrange_node:isVisible() then
                ...
                if self.back_panel then
                    if self.back_panel_ex_params then 
                        if self.back_panel == "campaign_event_msgbox" then 
    ...
    self.skill_percent_img:addTouchEventListener(function(widget, event_type)
        if event_type == ccui.TouchEventType.ended then
            audio_manager:PlayEffect("click")
            self:SetMoveLineupBg(not self.lineup_move_flag)
        end
    end)
    ...
    touch_listener:registerScriptHandler(function(touches, event)
        ...
        --限定区域
        if self.change_formation_begin_pos.y > 209 and self.change_formation_begin_pos.y < 765 and not self.finish_change_formatio_transition then
            self.scroll_view:setTouchEnabled(false)
