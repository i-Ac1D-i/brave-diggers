...
function social_event_panel:Show(user_id)
    self.root_node:setVisible(true)
    
    self:LoadFormationInfo()

    self.user_id = user_id or self.user_id
    self:ShowFriendTroop(self.user_id)
end
...
function social_event_panel:ShowFriendTroop(cur_friend_id)
    self.root_node:setVisible(true)
    local troop_info = social_logic:GetFriendTroopInfo(cur_friend_id)
    ...
    local touchlistener = function (widget, event_type)
        ...
        if event_type == ccui.TouchEventType.ended then
            ...
            if tag == self.formation_btn:getTag() then
                ...
                graphic:DispatchEvent("show_world_sub_scene", "formation_sub_scene", trans_type, true, mode, back_panel)
