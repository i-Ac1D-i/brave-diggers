...
local MERCENARY_QUALITY_STR =
{
    lang_constants:Get("mercenary_quality1"),
    lang_constants:Get("mercenary_quality2"),
    lang_constants:Get("mercenary_quality3"),
    lang_constants:Get("mercenary_quality4"),
    lang_constants:Get("mercenary_quality5"),
    lang_constants:Get("mercenary_quality6"),
}
...
function panel_util:LoadCostResourceInfo(config, sub_panels, pos_y, widget_num, pos_x)
    local cost_type_num = 0

    local resource_is_enough = true
    for i = 1, #resource_params do
        local need_resource_num = config[resource_params[i]]

        if need_resource_num and need_resource_num > 0 then
            cost_type_num = cost_type_num + 1

            local resource_type = constants.RESOURCE_TYPE[resource_params[i]]
            if cost_type_num > widget_num then
                return
            end

            sub_panels[cost_type_num]:Show(constants.REWARD_TYPE["resource"], resource_type, need_resource_num, true, false)
    ...
    return cost_type_num, resource_is_enough
end
...
function panel_util:SetMercenaryNameColor(widget, mercenary_template, is_show_name)
    ...
    if is_show_name then
        ...
        widget:setString(lang_constants:Get(MERCENARY_QUALITY_STR[quality]))
    end
...
function panel_util:SortMercenary(sort_type, mercenary_list)
