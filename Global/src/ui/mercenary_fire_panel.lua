...
function mercenary_fire_panel:CreateMercenarySubPanels()
    ...
    local source = constants.MERCENARY_TEMPLATE_SOURCE
...
function mercenary_fire_panel:Show()
    ...
    self:BackupCanFireMercenaryList()
...
function mercenary_fire_panel:BackupCanFireMercenaryList()
    ...
    for mercenary_id, mercenary in pairs(mercenary_list) do
        if mercenary.formation_info == 0 and not mercenary.is_leader then
...
function mercenary_fire_panel:FilterMercenary(quality)
    --重置
    for i = #self.tobe_fired_id_list, 1, -1 do
        self:RemoveTobeFiredInstanceId(self.tobe_fired_id_list[i], i)
    end
    ...
    for i = self.mercenary_num, 1, -1  do
        ...
        if mercenary.template_info.quality <= quality then
            if mercenary.level == 1 and mercenary.weapon_lv == 0 and mercenary.wakeup == 1 and mercenary.contract_lv == 0 then
                ...
                --金将以下的品质，解雇的碎片在500以上才可以解雇
                local can_fire = false
                if mercenary.template_info.quality == 5 then
                    can_fire = true
                ...
                if can_fire then
                    fire_num = fire_num + 1
                    table.insert(self.tobe_fired_id_list, mercenary.instance_id)
...
local sum_soul_chip = 0
    ...
    local quality = 0;
    for i, id in ipairs(self.tobe_fired_id_list) do
        ...
        if mercenary then
            ...
            soul_bone[mercenary.template_info.quality] = soul_bone[mercenary.template_info.quality] + mercenary.template_info.soul_bone
...
function mercenary_fire_panel:RemoveTobeFiredInstanceId(instance_id, id_index)
    ...
    if self.tobe_fired_id_list[id_index] then
        table.remove(self.tobe_fired_id_list, id_index)
    end
...
function mercenary_fire_panel:ChooseMercenary()
    ...
    if is_already_select then
        ...
        --选择
        table.insert(self.tobe_fired_id_list, self.cur_mercenary_id)

        if #self.tobe_fired_id_list > #self.select_sub_panels then
            self:CreateSelectPanels()
        end
    ...
    self.fire_num_text:setString(#self.tobe_fired_id_list)
    --r2 
    --r2修改右对齐
...
function mercenary_fire_panel:RegisterWidgetEvent()
    ...
    self.show_confirm_msgbox_btn:addTouchEventListener(function(widget, event_type)
        if event_type == ccui.TouchEventType.ended then
            ...
            local fire_num = #self.tobe_fired_id_list
            ...
            graphic:DispatchEvent("show_world_sub_panel", "mercenary_confirm_fire_panel", #self.tobe_fired_id_list, self.sum_soul_chip, self.sum_exp, self.tobe_fired_id_list,self.soul_bone,self.quality)
        end
    end)
