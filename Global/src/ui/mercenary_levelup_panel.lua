...
function mercenary_sub_panel:Init(root_node, index)
    ...
    self.desc4_text = self.levelup1_btn:getChildByName("desc4")
    self.bp_icon_img = self.levelup1_btn:getChildByName("bp_icon")
    self.artifact_icon_img = self.levelup1_btn:getChildByName("artifact_icon")
...
function mercenary_sub_panel:Load(mercenary)
    ...
    if mercenary.force_lv == constants["MAX_FORCE_LEVEL"] then
        ...
        self.wakeup_text:setVisible(false)
        self.wakeup_img:setVisible(false)
...
function mercenary_sub_panel:LoadLevelupInfo(mercenary)
    ...
    self.desc4_text:setVisible(false)
    ...
    elseif mercenary.level <= constants["MAX_LEVEL"] then
        if mercenary.wakeup < mercenary.template_info.max_wakeup then
            ...
            --检测资源, 设置按钮的颜色
            if not resource_logic:CheckResourceNum(RESOURCE_TYPE["gold_coin"], config.gold_coin) then
                self.levelup1_btn:setColor(panel_util:GetColor4B(0x929292))
                return
            end

            if not resource_logic:CheckResourceNum(config.resource_id, config.resource_num) then
                self.levelup1_btn:setColor(panel_util:GetColor4B(0x929292))
                return
            end
            ...
            if mercenary.level == constants["MAX_LEVEL"] then
                self.bp_add_text:setString("MAX")
...
function mercenary_sub_panel:LoadForgeWeaponInfo(mercenary)
    ...
    if mercenary.is_leader then
        ...
        if weapon_lv == constants["CAN_OPEN_ARTIFACT_WEAPON_LV"] and not mercenary.is_open_artifact and mercenary.template_info.have_artifact then
            is_open_artifact = true
        end

        self:SetWeaponBtnImgs(is_open_artifact)

        if is_open_artifact then
            if troop_logic:CheckOpenArtifactResource() then
                self.levelup1_btn:setColor(panel_util:GetColor4B(0xffffff))
            else
                self.levelup1_btn:setColor(panel_util:GetColor4B(0x929292))
            end
...
--设定武器图片
function mercenary_sub_panel:SetWeaponBtnImgs(is_open_artifact)
    self.desc1_text:setVisible(not is_open_artifact)
    self.desc2_text:setVisible(not is_open_artifact)
    self.desc4_text:setVisible(is_open_artifact)
    self.bp_add_text:setVisible(not is_open_artifact)
    self.bp_icon_img:setVisible(not is_open_artifact)
    self.artifact_icon_img:setVisible(is_open_artifact)
...
function mercenary_levelup_panel:Show()
    self.troop_bp_text:setString(tostring(troop_logic.battle_point))
