...
function fund_panel:Show(config, step_index)
    ...
    self.profit_text:setString(self:GetLocaleInfoString(self.config, "mult_str1", step_index))
...
function fund_panel:GetLocaleInfoString( cur_config, key, index )
...
function fund_panel:GetDesc()
    ...
    local locale = platform_manager:GetLocale()
    local result = first_conf["desc"]
    if first_conf["desc".."_"..locale] then
        result = first_conf["desc".."_"..locale]
    end
    return string.format(result, bd_num, conf.profit_duration-1, unpack(nums))
end
    ...
    self.invest_btn:addTouchEventListener(function(widget, event_type)
        ...
        if event_type == ccui.TouchEventType.ended then
            ...
            if info.cur_value_multi[self.step_index] == 0 then
                ...
                --查看领取界面
                graphic:DispatchEvent("show_world_sub_panel", "carnival.take_profit_msgbox", self.config, self.step_index)
            end
        end
    end)
end
