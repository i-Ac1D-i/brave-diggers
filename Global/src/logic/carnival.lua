...
--返回第一个在满足条件的活动
function carnival:GetSpecialCarnival(template_type)  --89  sns_invite
...
function carnival:GetSpecialVisibleStyle()
    ...
    local CARNIVAL_TYPE = constants.CARNIVAL_TYPE
    local config = self:GetSpecialCarnival(CARNIVAL_TEMPLATE_TYPE["christmas"], CARNIVAL_TYPE["collect_item"])
...
--领取奖励, 最后一个参数为忽略检测状态
function carnival:TakeReward(config, step_index, cant_check_status)
    if not self:CanTakeReward(config, step_index, cant_check_status) then
        return
    end
    
    network:Send({take_carnival_reward = {key = config.key, step = step_index }} )
end
...
function carnival:RegisterMsgHandler()
    ...
    network:RegisterEvent("update_carnival_info_ret", function(recv_msg)
        print("update_carnival_info_ret")
