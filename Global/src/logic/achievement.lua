...
function achievement:CheckAchievementStatus(achievement_type)
    ...
    if not achievement_config[achievement_type] then
        return
    end

    local next_step_config = achievement_config[achievement_type][cur_step + 1]
...
function achievement:Complete(achievement_type)
    --检测背包
    local cur_step = self.step_list[achievement_type]
    local reward_type = tonumber(achievement_config[cur_step + 1]["reward_type"])
    local sub_id = tonumber(achievement_config[cur_step + 1]["param1"])
    local num = tonumber(achievement_config[cur_step + 1]["param2"])
