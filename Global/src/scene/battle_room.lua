...
function battle_fragment:Init()
    ...
    update_methods[FRAGMENT_PART["cast_action"]] = function(elapsed_time, battle_room)
        ...
        if not self.caster:IsPlayAction() and self.pic_fade_out_remain_time <= 0 then
            ...
            if self.caster_troop_id == LEFT_TROOP_ID then
                local bp1_text, x, y = battle_room.ui_root:UpdateBattlePoint(RIGHT_TROOP_ID, self.receiver.cur_bp)
                battle_room.skill_bp1_tracker:Bind("hp", x, y, bp1_text)
            else
                local bp1_text, x, y = battle_room.ui_root:UpdateBattlePoint(LEFT_TROOP_ID, self.receiver.cur_bp)
                battle_room.skill_bp1_tracker:Bind("hp", x, y, bp1_text)
            end
    ...
    update_methods[FRAGMENT_PART["receiver_effect_and_action"]] = function(elapsed_time, battle_room)
        ...
        if not self.receiver:IsPlayAction() and self.role_effect_time <= 0 then
            ...
            if self.caster_troop_id == LEFT_TROOP_ID then
               local bp2_text, x, y = battle_room.ui_root:UpdateBattlePoint(LEFT_TROOP_ID, self.caster.cur_bp)
                battle_room.skill_bp2_tracker:Bind("hp", x, y, bp2_text)

            else
                local bp2_text, x, y = battle_room.ui_root:UpdateBattlePoint(RIGHT_TROOP_ID, self.caster.cur_bp)
                battle_room.skill_bp2_tracker:Bind("hp", x, y, bp2_text)
            end
...
function battle_room:CreateSpineNodes()
    ...
    self.start_spine_node:registerSpineEventHandler(function(event)
        ...
        elseif event.animation == "win" then
        ...
        elseif event.animation == "lose" then
        ...
        elseif event.animation == "win_end" or event.animation == "lose_end" and event.loopCount == 1 then
            --self.ui_root:SetLeave(true)
            --self.ui_root:ShowLeaveBtn()
    ...
    self.start_spine_node:registerSpineEventHandler(function(event)
        ...
        elseif event.eventData.name == "touch" then 
           self.ui_root:SetLeave(true)
        elseif event.eventData.name == "button_in" then
            -- self.ui_root:ShowBalanceDescText()
        end
...
function battle_room:Show(battle_type, data, property, record, result, callback)
    ...
    local left_max_bp, right_max_bp, left_cur_bp, right_cur_bp = string.match(property, "(%d+):(%d+)#(%d+):(%d+)#")
    ...
    if battle_type == BATTLE_TYPE["vs_monster"] or 
        ...
        battle_type == BATTLE_TYPE["vs_campaign"] then
    ...
    elseif battle_type == BATTLE_TYPE["vs_arena_player"] or battle_type == BATTLE_TYPE["vs_ladder_player"] then
        ...
        for i = 1, num do
            local template_info = mercenary_config[rival_info.template_id_list[i]]
    ...
    --         local template_info = mercenary_config[player2.template_id_list[i]]
    --         self.monster_sprite_list[i] = template_info.sprite
    --     end
    ...
    self.left_troop:SetProperty(tonumber(left_max_bp), tonumber(left_cur_bp), self.right_troop, speed1, defense1, dodge1, authority1, original_speed1, original_defense1, original_dodge1, original_authority1)
    self.right_troop:SetProperty(tonumber(right_max_bp), tonumber(right_cur_bp), self.left_troop, speed2, defense2, dodge2, authority2, original_speed2, original_defense2, original_dodge2, original_authority2)
