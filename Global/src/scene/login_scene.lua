...
function login_scene:ctor(switch_account)
    ...
    self:registerScriptHandler(function(event)
        if event == "enter" then
            ...
            feature_config:Init(platform_manager:GetChannelInfo())
            ...
            self.random_num = tostring(math.random(1, 3))
...
function login_scene:RegisterEvent()
    ...
    graphic:RegisterEvent("update_app_msgbox", function(url)
        ...
        -- title = "Update"
        -- desc = "Please download the latest App from Store."
        -- confirm_txt = "Update"
        -- cancel_txt = "Closed"
